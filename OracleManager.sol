...
contract OracleManager is IOracleManager, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function generatePerformance(address validator) external whenNotPaused onlyRole(OPERATOR_ROLE) returns (bool) {
        ...
        require(
            block.timestamp >= lastValidatorUpdate[validator] + MIN_UPDATE_INTERVAL, "Update too frequent for validator"
        );
        ...
        for (uint256 i = 0; i < oracleCount;) {
            ...
            try IOracleAdapter(oracle).getPerformance(validator) returns (
                uint256 balance,
                uint256 performanceScore, // Just one score instead of 4
                ...
                // Sanity checks
                if (performanceScore <= maxPerformanceBound) {
                    // Aggregate scores
        ...
        uint256 avgPerformanceScore = totalPerformanceScore / validOracleCount;
        ...
        if (address(sanityChecker) != address(0)) {
            ...
            (valid, reason) = sanityChecker.checkValidatorSanity(
                validator, avgBalance, avgPerformanceScore, avgRewardAmount, avgSlashAmount
            );
        }
        ...
        validatorManager.updateValidatorPerformance(
            validator,
            avgBalance,
            avgPerformanceScore // Single score only
        );
    ...
    function setMaxPerformanceBound(uint256 newBound) external onlyRole(MANAGER_ROLE) {
