    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _distributeStake(uint256 amount, OperationType operationType) internal {
        ...
        if (operationType == OperationType.UserDeposit) {
            ...
            // @chi current buffer is not enough
            if (amount > 0 && currentBuffer < target) {
                // @chi calculate diff
                uint256 bufferSpace = target - currentBuffer;
                // @chi calculate how much `amount` will be leaved in this contract
                uint256 amountToBuffer = Math.min(amount, bufferSpace);
                hypeBuffer = currentBuffer + amountToBuffer; // @chi new current hype Buffer
                // @chi new amount (after minus amountToBuffer) will deposit to L1 (add op to queue)
            ...
            // @chi there are two L1 ops (sendCDeposit & sendTokenDelegate), but function executes sendCDeposit immediately
            // @chi then adds sendTokenDelegate to queue instead
     ...
     * @chi Queue l1 operation is main action, operationType might be `UserWithdrawal` or `RebalanceWithdrawal`
     */
     ...
     * @chi off-chain trigger, add operations to queue manually
     */
    ...
    function _queueL1Operation(address validator, uint256 amount, OperationType operationType) internal {
        ...
        for (uint256 i = startIndex; i < queue.length; i++) {
            if (queue[i].validator == validator && queue[i].operationType == operationType) {
        ...
        queue.push(L1Operation({validator: validator, amount: amount, operationType: operationType}));
     ...
     * @chi process == execute, the function will interact with L1
     * @chi off-chain trigger
     */
    ...
    function cancelWithdrawal(address user, uint256 withdrawalId) external onlyRole(MANAGER_ROLE) whenNotPaused {
        ...
        uint256 kHYPEAmount = request.kHYPEAmount;
        uint256 kHYPEFee = request.kHYPEFee;
        uint256 bufferUsed = request.bufferUsed;

        // Check kHYPE balances
        require(kHYPE.balanceOf(address(this)) >= kHYPEAmount + kHYPEFee, "Insufficient kHYPE balance");

        // Restore buffer if buffer was used for this withdrawal
        if (bufferUsed > 0) {
            hypeBuffer += bufferUsed;
            emit BufferIncreased(bufferUsed, hypeBuffer);
        }
