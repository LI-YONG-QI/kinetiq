...
contract StakingAccountant is IStakingAccountant, Initializable, AccessControlEnumerableUpgradeable {
     ...
     */
    function authorizeStakingManager(address manager, address kHYPEToken) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(manager != address(0), "Invalid manager address");
        ...
        // Make sure only one toke per instance
        require(_uniqueTokens.length() == 1, "too many token registed");
    ...
    function _getExchangeRatio() internal view returns (uint256) {
        ...
        // Calculate total HYPE (all in 18 decimals)
